...
### a. Use Case Diagram
<img src="use case diagram.png" alt="Use Case Diagram" width="600"/>
Admin bertindak sebagai pemegang kendali utama yang bertanggung jawab atas pengelolaan seluruh infrastruktur data, termasuk pemeliharaan tabel users, siswa, dan kelas dalam database agar sistem tetap berjalan optimal. Selain melakukan login, Admin memiliki tugas krusial yaitu melakukan generate absensi setiap harinya agar daftar nama siswa tersedia untuk diisi. 
Di sisi operasional, Staff atau Guru berperan dalam memantau jalannya absensi dengan hak akses untuk melakukan login dan melihat data secara real-time. Mereka juga memiliki wewenang untuk melakukan intervensi berupa pengeditan data jika ditemukan kesalahan input, serta membantu fungsi Admin dalam menyiapkan daftar hadir harian (generate) agar proses belajar mengajar dapat dimulai tepat waktu.
Kepala Sekolah menempati posisi pengawas yang hanya memerlukan akses terbatas untuk login dan melihat laporan kehadiran.
Siswa berperan sebagai pengguna akhir yang berinteraksi langsung dengan sistem melalui fitur absen mandiri. Setiap kali siswa mencatatkan kehadirannya, data tersebut akan terekam secara otomatis ke dalam tabel absensi berdasarkan identitas unik mereka, yang nantinya akan disusun rapi dalam kolom-kolom laporan seperti NIS, nama, dan status (H/S/I/A) oleh sistem.
